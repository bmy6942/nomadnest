// seed.js — 純 JavaScript 版本，Windows/Mac/Linux 均可直接執行
...
async function main() {
  ...
  const tenant2 = await prisma.user.create({
    data: { name: 'Thomas Müller', email: 'thomas@test.com', password: pwd, role: 'tenant', verified: true, phone: '0977333444', bio: 'German software engineer, Taiwan Gold Card holder.' },
  });
  ...
  const listings = [
    {
      ...
      amenities: JSON.stringify(['冷氣', '洗衣機', '冰箱', '微波爐', '熱水器']),
    ...
    },
    {
      ...
      description: '台北信義區精品整層公寓，兩房一廳，適合一人辦公或兩位游牧者同住分攤費用。客廳有一整面牆的白板可規劃，網路為企業級光纖，實測900Mbps，附備用4G熱點。距離象山捷運站步行7分鐘。',
      ...
      amenities: JSON.stringify(['冷氣', '洗衣機', '冰箱', '白板牆', '備用4G']),
      images: JSON.stringify(['https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=800']),
      status: 'active', foreignOk: true, ownerId: landlord1.id,
    },
    {
      title: '中山區日式和風雅房 — 近松山機場',
      description: '中山區精緻日式裝潢雅房，空間設計感十足，採光佳。網路為光纖200Mbps，穩定可靠。距離松山機場10分鐘車程，對需要頻繁出差的商務游牧者極為方便。',
    ...
    },
    {
      ...
      description: '台中西區最受歡迎的游牧者共居空間！整棟5層樓，1F為共用工作室（附印表機、白板、投影機）。所有房間均配備光纖網路及人體工學工作桌。距離審計新村步行5分鐘。定期舉辦游牧者交流活動。',
      ...
      amenities: JSON.stringify(['冷氣', '洗衣機', '冰箱', '共用工作室', '投影機', '白板']),
    ...
    {
      title: '北區超值套房 — 含費，近逢甲，高 CP 值',
      description: '台中北區整理得相當乾淨的套房，水電網路全含，CP值極高。日常遠端工作、視訊會議完全夠用。距逢甲夜市步行10分鐘，生活機能好。適合預算有限的游牧新手。',
    ...
    },
    {
      ...
      description: 'Hi foreign nomads! This is a clean and bright studio in Lingya District, Kaohsiung. 500Mbps fiber internet, dedicated work desk, fully furnished. Walking distance to Light Rail. Landlord speaks English. All bills included.',
      ...
      includedFees: JSON.stringify(['水費', '電費', '網路費', '管理費']),
      amenities: JSON.stringify(['冷氣', '洗衣機', '冰箱', '微波爐']),
    ...
    },
    {
      ...
      description: '高雄前鎮區寬敞套房，坪數大、格局方正。高捷三多商圈站步行8分鐘。2個月以上優惠月租NT$11,500。附近有家樂福、好市多，生活採買極為方便。',
    ...
    },
    {
      ...
      description: '花蓮市難得一見的景觀套房！打開窗戶即見中央山脈，環境安靜舒適。光纖網路實測300Mbps，工作毫無壓力。附免費腳踏車借用。不定期有游牧者小聚活動。',
    ...
    {
      title: '板橋區超高速纖維套房 — 近台北車站',
      description: '新北板橋區CP值極高！距離板橋車站步行5分鐘，台鐵、捷運、高鐵三鐵共構，往來台北、桃園機場極為方便。網路企業級光纖實測600Mbps。',
    ...
    {
      title: '永和區安靜住宅套房 — 適合創作者',
      description: '新北永和區住宅區的安靜套房，環境極為安靜，非常適合需要專注創作的工作者。光纖100Mbps，附近公園多，適合散步放鬆。',
      ...
      includedFees: JSON.stringify(['水費', '網路費']),
      amenities: JSON.stringify(['冷氣', '洗衣機共用', '廚房共用']),
      images: JSON.stringify(['https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=800']),
      status: 'active', foreignOk: false, ownerId: landlord2.id,
    },
    {
    ...
    {
      title: '左營區捷運站旁套房（待審核）',
      description: '高雄左營區，高鐵路近捷運左營站，測試用房源。',
  ...
  if (activeListings.length >= 2) {
    ...
    await prisma.application.create({
      data: {
        ...
        message: "Hi! I'm Thomas, a German software engineer with Taiwan Gold Card. Looking for 2 months stay starting May 15.",
  ...
  if (activeListings.length >= 1) {
    await prisma.review.create({
      data: {
        ...
        content: '非常推薦！Wi-Fi速度真的如廣告所示，遠端工作零壓力。房東很好溝通，環境乾淨整潔，會再租！',
      },
    });
  }
  ...
  console.log('  🏠 房東3   landlord3@test.com / test123');
