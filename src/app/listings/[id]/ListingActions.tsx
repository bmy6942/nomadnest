...
export default function ListingActions({ listingId, ownerId, price, isLoggedIn, isTenant, hasApproved, hasReviewed, pendingMoveInDate }: Props) {
  ...
  // 存在中的預約 status（用來顯示正確的提示文字）
  const [existingViewingStatus, setExistingViewingStatus] = useState<string | null>(null);

  // ✅ mount 時查詢此房源是否已有進行中的看房預約（防止重複提交 + 跨頁重整保留狀態）
  useEffect(() => {
    if (!isTenant) return;
    fetch('/api/viewings').then(r => r.ok ? r.json() : []).then((list: { listingId: string; status: string }[]) => {
      const found = list.find(v => v.listingId === listingId && (v.status === 'pending' || v.status === 'confirmed'));
      if (found) { setViewingDone(true); setExistingViewingStatus(found.status); }
    }).catch(() => {});
  }, [listingId, isTenant]);
  ...
  const openConversation = async () => {
    ...
    if (res.ok) {
      ...
      setContactError(data.error || '無法開啟對話，請稍後再試');
    }
  };

  const submitViewing = async () => {
    if (!times.t1) { setViewingError('請至少填寫一個候選時間'); return; }
    ...
    if (res.ok) {
      setViewingDone(true);
      setExistingViewingStatus('pending');
      setShowViewing(false);
    } else {
      const d = await res.json().catch(() => ({})) as { error?: string };
      setViewingError(d.error || '預約失敗，請稍後再試');
    }
  };

  const submitReview = async () => {
    const trimmed = review.content.trim();
    if (!trimmed) { setReviewError('請填寫評價內容'); return; }
    if (trimmed.length < 10) { setReviewError(`評價內容至少需要 10 個字元（還差 ${10 - trimmed.length} 字）`); return; }
    if (trimmed.length > 2000) { setReviewError('評價內容不可超過 2000 個字元'); return; }
    ...
    if (res.ok) {
      ...
      const d = await res.json().catch(() => ({})) as { error?: string };
      setReviewError(d.error || '評價提交失敗，請稍後再試');
    }
  };
    ...
    <div className="space-y-3 mt-4">
        ...
        <>
          ...
          {contactError && (
            <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 flex items-center justify-between">
              ...
              <button onClick={() => setContactError('')} className="ml-2 text-red-400 hover:text-red-600 leading-none">×</button>
            </div>
          )}
          ...
          ) : existingViewingStatus === 'confirmed' ? (
            <div className="text-center text-sm text-green-700 bg-green-50 border border-green-200 rounded-xl py-2.5">
              ✅ 看房時間已確認！請前往控制台查看詳情
            </div>
          ) : (
            <div className="text-center text-sm text-blue-700 bg-blue-50 border border-blue-200 rounded-xl py-2.5">
              📅 看房預約已送出，等待房東確認
            </div>
          )}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-3">
          ...
          {viewingError && (
            <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 flex items-center justify-between">
              ...
              <button onClick={() => setViewingError('')} className="ml-2 text-red-400 hover:text-red-600 leading-none">×</button>
            </div>
          )}
          <div className="flex gap-2">
            ...
            <button onClick={() => { setShowViewing(false); setViewingError(''); }}
      ...
      {showReview && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">評價內容 <span className="text-gray-400">（至少 10 字）</span></label>
            <textarea value={review.content} onChange={e => setReview(r => ({ ...r, content: e.target.value }))}
              rows={3} maxLength={2000} placeholder="分享你的入住體驗…" className="input text-sm resize-none" />
            <div className="flex items-center justify-between mt-1">
              {review.content.trim().length > 0 && review.content.trim().length < 10 && (
                <span className="text-xs text-red-500">還需 {10 - review.content.trim().length} 個字元</span>
              )}
              {review.content.trim().length >= 10 && <span className="text-xs text-green-600">✓</span>}
              {review.content.trim().length === 0 && <span />}
              <span className={`text-xs ${review.content.length > 1800 ? 'text-amber-500' : 'text-gray-400'}`}>
                {review.content.length}/2000
              </span>
            </div>
          </div>
          {reviewError && (
            <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 flex items-center justify-between">
              ...
              <button onClick={() => setReviewError('')} className="ml-2 text-red-400 hover:text-red-600 leading-none">×</button>
            </div>
          )}
          <div className="flex gap-2">
            ...
            <button onClick={() => { setShowReview(false); setReviewError(''); }}
              className="btn-secondary flex-1 py-2 text-sm">取消</button>
          </div>
        </div>
      )}
